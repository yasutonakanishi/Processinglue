...
public class Segment {
	...
	public void setupGL(GL2 gl2, Debug debug, PVector[] p, PVector[] t, PVector[] n) {
		...
		gl2.glGenBuffers(1, glbuf, 0);

		dataFB = setupFloatBuffer(f);

		bindThisBuffer(gl2, glbuf[0], f, dataFB);
	...
	public void beginDrawGL(GL2 gl2) {
		gl2.glBindBuffer(GL2.GL_ARRAY_BUFFER, glbuf[0]);
		...
		gl2.glVertexPointer(3, GL2.GL_FLOAT, 32, 0);
		gl2.glTexCoordPointer(2, GL2.GL_FLOAT, 32, 12);
		gl2.glNormalPointer(GL2.GL_FLOAT, 32, 20);
	...
	public void drawGL(GL2 gl2, int GLTYPE) {
		...
		// indexIB);
		gl2.glDrawArrays(GLTYPE, 0, indexIB.capacity());

		endDrawGL(gl2);
	}

	public void endDrawGL(GL2 gl2) {
	...
	private void bindThisBuffer(GL2 gl, int num, float[] f, FloatBuffer FB) {
		gl.glBindBuffer(GL2.GL_ARRAY_BUFFER, num);
		gl.glBufferData(GL2.GL_ARRAY_BUFFER, 4 * f.length, FB, GL2.GL_STATIC_DRAW);
	}
}