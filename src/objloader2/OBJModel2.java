...
public class OBJModel2 {
	...
	// global variables
	protected ArrayList<PVector> vertices; // vertexes
	protected ArrayList<PVector> textureVertices; // texture coordinates
	protected ArrayList<PVector> normalVertices; // normals
	protected ArrayList<Segment> segments;

	protected Hashtable<String, Material2> materials;
	protected Hashtable<String, Group> groups;

	protected String name = "default";

	protected String defaultMaterialName = "default";

	protected Group defaultGroup = new Group("default");
	protected Segment defaultSegment = new Segment();

	protected PApplet parent;
	protected PImage texture; // texture image applied from the code.

	// runtime rendering variables
	protected int drawMode = PConstants.TRIANGLES; // render mode (ex. POLYGON,
	// POINTS ..)

	protected boolean useTexture = true;
	protected boolean useMaterial = true;
	...
	protected String originalTexture;
	 ...
	 */
	public OBJModel2(PApplet _parent) {
		setup(_parent);
	}
	 ...
	 */
	public OBJModel2(PApplet _parent, String _filename) {
		setup(_parent);
		load(_filename);
	}
	 ...
	 * </br>
	 */
	public OBJModel2(PApplet _parent, String _fileName, int _shapeMode) {
		setup(_parent);
	 ...
	 */
	public OBJModel2(PApplet _parent, String _fileName, String _texturePathMode) {
		setup(_parent);
	 ...
	 * </br>
	 */
	public OBJModel2(PApplet _parent, String _fileName, String _texturePathMode, int _shapeMode) {
		setup(_parent);
		texturePathMode = _texturePathMode.toLowerCase();
		shapeMode(_shapeMode);
		load(_fileName);
	...
	private void setup(PApplet parent) {
		...
		materials = new Hashtable<String, Material2>();
	...
	public void setupGL() {
		...
		PGL pgl =  ((PGraphicsOpenGL) parent.g).beginPGL();		  
		gl2 = PGL.gl.getGL2();
		...
		for (int i = 0; i < segments.size(); i++) {
			...
			if (tmpModelSegment.getFaceCount() != 0) { // Why are there empty
				...
				tmpModelSegment.setupGL(gl2, debug, vs, tVs, nVs);

				mtl = materials.get(tmpModelSegment.materialName);
				mtl.setupGL(gl2, debug);
			}
		}
	...
	public void drawGL() {
		...
		for (int i = 0; i < segments.size(); i++) {
			...
			if (tmpModelSegment.getFaceCount() != 0) { // again with the
				...
				mtl.beginDrawGL(gl2, useMaterial, useTexture);

				switch (drawMode) {

				case (PConstants.POINTS):
					tmpModelSegment.drawGL(gl2, GL2.GL_POINTS);
					break;

				case (PConstants.LINES):
					tmpModelSegment.drawGL(gl2, GL2.GL_LINES);
					break;

				case (PConstants.TRIANGLES):
					tmpModelSegment.drawGL(gl2, GL2.GL_TRIANGLES);
					break;

				case (PConstants.TRIANGLE_STRIP):
					tmpModelSegment.drawGL(gl2, GL2.GL_TRIANGLE_STRIP);
					break;

				case (PConstants.QUADS):
					tmpModelSegment.drawGL(gl2, GL2.GL_QUADS);
					break;

				case (PConstants.QUAD_STRIP):
					tmpModelSegment.drawGL(gl2, GL2.GL_QUAD_STRIP);
					break;

				case (PConstants.POLYGON):
					tmpModelSegment.drawGL(gl2, GL2.GL_POLYGON);
					break;

				}

				mtl.endDrawGL(gl2, useMaterial, useTexture);
	...
	//private void parseOBJ(String[] bread) {	
		try {
			...
			Material2 defaultMaterial = new Material2();
			...
			while ((line = bread.readLine()) != null) {
				...
				// The below patch/hack comes from Carles Tom? Mart?and is a
				...
				if(line.length() > 0){
					...
					if (elements.length > 0) {
						...
						if (elements[0].equals("newmtl")) {
							...
							Material2 tmpMtl = new Material2();
